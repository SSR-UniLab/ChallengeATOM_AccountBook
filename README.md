# ChallengeATOM_AccountBook
This repository is for account book on UniLab challenge for "Challenge ATOM."

## 管理方法

- 帳簿の実データは [ledger/book.csv](ledger/book.csv) で管理します（Excelでも編集可）。
  - 文字化け防止のためCSVはUTF-8(BOM付き)で保存しています。Excelで編集後保存する際は「名前を付けて保存」→ファイルの種類を「CSV UTF-8(コンマ区切り)」にしてください（通常の「CSV(コンマ区切り)」はShift-JISで保存され文字化けします）。
- 領収書などの証拠書類は [evidence/](evidence/) フォルダに、CSVの「証拠書類の番号」に対応するファイル名（例: `001_領収書.pdf`）で格納します。

- CSVを更新したら以下を実行すると、下記「現在の帳簿」の表が自動更新されます。

```sh
python scripts/update_readme.py
```

- リポジトリ初回セットアップ時に以下を1回実行しておくと、`ledger/book.csv` を含むコミット時に自動でREADME.mdが更新されます（pre-commitフック）。

```sh
git config core.hooksPath scripts/hooks
```

### CSVのフォーマット

| 列名 | 内容 |
| --- | --- |
| 日付 | YYYY-MM-DD |
| 費目 | 物品費 / 交通費 / 謝金 / その他 / 入金 |
| 内容・購入物 | 何を購入・支払したか |
| 用途 | 開発のどこに使ったか |
| 金額(税込) | 数値のみ（カンマ区切りは可、その場合は "1,000" のように引用符で囲む） |
| 口座入金 | 口座への入金額（数値のみ） |
| 口座出金 | 口座からの出金額（数値のみ） |
| 未払金（クレジットカード支払い） | クレジットカード等の未払い額（数値のみ） |
| 支払先 | 支払先の名称 |
| 証拠書類の番号 | evidence/ 内のファイルに対応する番号 |

「現在の帳簿」の表には従来どおり `日付 / 費目 / 内容・購入物 / 用途 / 金額(税込) / 支払先 / 証拠書類の番号` の7列のみを出力します。費目が「入金」の行は支出ではないため、表および合計から除外されます。

## 現在の帳簿

<!-- LEDGER:START -->
| 日付 | 費目 | 内容・購入物 | 用途 | 金額(税込) | 支払先 | 証拠書類の番号 |
| --- | --- | --- | --- | ---: | --- | --- |
| 2026/7/25 | 物品費 | アルミフレーム・キャスタ | ロボットの足回りとして | 7,612円 | misumi | 1 |
| 2026/8/3 | 物品費 | ロボットアームB601-RS | ロボット搭載用のロボットアームとして | 289,596円 | SeeedStudio | 2 |
| 2026/8/3 | 物品費 | 機械学習向けノートパソコン | ロボット搭載、学習用のパソコンとして | 285,100円 | ドスパラ | 3 |
| 2026/8/24 | 物品費 | 機械学習用カメラ | ロボットアーム制御用として | 16,829円 | Amazon | 4 |
| 2026/8/27 | 物品費 | ロボットアーム、税関分 | 購入付帯関税 | 29,050円 | OCS | 3 |
| 2026/8/29 | 物品費 | クランプ、USBケーブル | ロボットアームを動作補助として | 1,762円 | Amazon | 5 |
| 2026/8/28 | 物品費 | ロボット用コントローラ | ロボットアーム | 3,300円 | Hardoff | 6 |
|  |  |  | **合計** | **633,249円** |  |  |
<!-- LEDGER:END -->
